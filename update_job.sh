...
docker run --user 0 -v ${MOL_APP_PATH}:/app -it deployment-mol_search bash -c "cd /app && python generate_database.py" && docker restart mol_search && echo "Successfuly updated mol_search" || (
...
docker run --user 0 -v ${TEXT_APP_PATH}:/app -it deployment-text_search bash -c "cd /app && python download_couples.py && python download_smiles.py && python download_taxa.py" && docker restart text_search && echo "Successfuly updated text_search" || (
